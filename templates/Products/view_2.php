...
<div class="container py-5">
    <div class="row">
        <div class="col-lg-5">
            <?= $this->element('galleries/3', ['images' => $product->gallery->gallery_images]) ?>
        </div>
        <div class="col-lg-4">
            <div class="d-flex flex-column justify-content-around">
                ...
                <?php if ($manufacturer = $product['manufacturer']) : ?>
                    <div class="mb-3">
                        <img src="<?= \ProductBackend\Core\Utility::getFileUrl($manufacturer['image_id'], 100, 70) ?>"
                             style="height: 60px">
                    </div>
                    <div>
            ...
            <?php foreach ($product['specification_groups'] as $groupName => $specificationGroup) : ?>
                <table class="table mb-0">
                    ...
                    <?php foreach ($specificationGroup as $specification) : ?>
                        <tr>
                            <td class="bg-4 border-black fw-bold"><?= $specification['name'] ?></td>
                            <td class="border-black"><?= $specification['text_value'] ?></td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>
